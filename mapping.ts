...
function generateTailwindClassFromCSS(css: string): string | undefined {
  ...
  for (const category of tailwindClasses.keys()) {
    const classes = tailwindClasses.get(category);
    if (!classes) continue;
    ...
    for (const classObj of classes) {
      ...
      if (classObj.compositeValues) {
        ...
        if (allPropertiesMatch) {
          const values = cssPropertiesEntries
            .map(([key, _]) => properties[key])
            .filter((value): value is string => Boolean(value));
          ...
          if (classObj.allowArbitrary && value) {
...
console.log(generateTailwindClassFromCSS('color: red;'));
