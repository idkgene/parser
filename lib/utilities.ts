...
export function getLineNumberFromPosition(
  ...
  for (var i = lineStartIndices.length - 1; i >= 0; i--)
    if (pos >= lineStartIndices[i]) return i + 1;
  return 0;
}

export function getColumnNumberFromPosition(
  pos: number,
  lineStartIndices: number[],
): number {
  for (var i = lineStartIndices.length - 1; i >= 0; i--) {
    var startIdx = lineStartIndices[i];
    if (pos >= startIdx) return pos - startIdx;
  }

  return pos;
}

export function getIndexFromLineColumn(
  lineNumber: number,
  columnNumber: number,
  lineStartIndices: number[],
): number {
...
export function insertRange(
  ...
  insertRange: T.ISourceRange,
): void {
  if (!range || !insertRange) return;
  ...
  if (
    oldStartLine === insertRange.startLine &&
    oldStartColumn >= insertRange.startColumn
  ) {
  ...
  if (
    oldEndLine === insertRange.startLine &&
    oldEndColumn > insertRange.startColumn
  ) {
    range.endLine += lineOffset;
    range.endColumn += columnOffset;
  } else if (oldEndLine > insertRange.startLine) range.endLine += lineOffset;
}
...
export function deleteRange(
  range: T.ISourceRange,
  deleteRange: T.ISourceRange,
): void {
  if (!range || !deleteRange) return;
  ...
  if (
    deleteRange.endLine < oldStartLine ||
    (deleteRange.endLine === oldStartLine &&
      deleteRange.endColumn < oldStartColumn)
  ) {
    ...
    if (deleteRange.endLine === oldStartLine) {
      ...
      if (oldStartLine === oldEndLine) range.endColumn -= columnSpan;
    }
  ...
  if (
    deleteRange.startLine > oldEndLine ||
    (deleteRange.startLine === oldEndLine &&
      deleteRange.startColumn > oldEndColumn)
  ) {
    // nothing to do...
    return;
  }
  ...
  range.endLine =
    oldEndLine < deleteRange.endLine
      ? deleteRange.startLine
      : Math.max(range.startLine, oldEndLine - lineSpan);
  ...
  if (oldEndLine === deleteRange.endLine)
    range.endColumn = Math.max(
      ...
      oldEndColumn + deleteRange.startColumn - deleteRange.endColumn,
    );
 ...
 * @param offset
 */
export function insertRangeFromNode(
  ast: T.INode,
  nodeModified: T.INode,
  offset?: T.ISourceRange,
): void {
  if (!ast || !nodeModified) return;
  ...
  var updateRecursive = function (node: T.INode) {
    if (node !== nodeModified) {
      ...
      } else insertRange(node.range, range);

      // recursively update the children
      var children = node.getChildren();
      var len = children.length;
      for (var i = 0; i < len; i++) updateRecursive(children[i]);
...
export function zeroRange(ast: T.INode, nodeModifiedOrRange: any): void {
  ...
  var r: T.ISourceRange, range: T.ISourceRange;
  ...
  if (
    nodeModifiedOrRange instanceof AST.ASTNode ||
    nodeModifiedOrRange instanceof Tokenizer.Token
  ) {
    ...
    nodeModifiedOrRange.startLine !== undefined &&
    nodeModifiedOrRange.startColumn !== undefined &&
    nodeModifiedOrRange.endLine !== undefined &&
    nodeModifiedOrRange.endColumn !== undefined
  ) {
    range = new AST.SourceRange(
      nodeModifiedOrRange.startLine,
      nodeModifiedOrRange.startColumn,
      nodeModifiedOrRange.endLine,
      nodeModifiedOrRange.endColumn,
    );
  } else return;
...
export function updateNodeRange(
  ast: T.INode,
  nodeModified: T.INode,
  newRange: T.ISourceRange,
): void {
  if (!nodeModified) return;
...
export function getRangeDifference(
  oldText: string,
  newText: string,
  ...
  var oldEndLine = getLineNumberFromPosition(
    oldText.length,
    oldTextStartIndices,
  );
  var oldEndColumn = getColumnNumberFromPosition(
    oldText.length,
    oldTextStartIndices,
  );
  var newEndLine = getLineNumberFromPosition(
    newText.length,
    newTextStartIndices,
  );
  var newEndColumn = getColumnNumberFromPosition(
    newText.length,
    newTextStartIndices,
  );
  ...
  return new AST.SourceRange(
    ...
    newEndLine === oldEndLine
      ? oldRange.endColumn + newEndColumn - oldEndColumn
      : newEndColumn,
  );
}
...
export function offsetRange(
  ...
  var offsetRecursive = function (node: T.INode /* , updateTrivia: boolean*/) {
    ...
    if (node.range.endLine === startLine) node.range.endColumn += columnOffset;
...
export function getTextFromRange(text: string, range: T.ISourceRange): string {
  ...
  var start = getIndexFromLineColumn(
    range.startLine,
    range.startColumn,
    lineStartIndices,
  );
  var end = getIndexFromLineColumn(
    range.endLine,
    range.endColumn,
    lineStartIndices,
  );
...
export function relativeRange(
  ...
  target: T.ISourceRange,
): T.ISourceRange {
  return {
    startLine: source.startLine - target.startLine,
    startColumn:
      source.startLine === target.startLine
        ? source.startColumn - target.startColumn
        : source.startColumn,
    endLine: source.endLine - target.startLine,
    endColumn:
      source.endLine === target.startLine
