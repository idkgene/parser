...
function isLetter(c: number): boolean {
  return (
    !isNaN(c) &&
    ((EChar.UCASE_A <= c && c <= EChar.UCASE_Z) ||
      (EChar.LCASE_A <= c && c <= EChar.LCASE_Z))
  );
}
...
function isSignificantWhitespace(t1: Token, t2: Token): boolean {
  ...
  if (token1 === EToken.IDENT) {
    return (
      token2 === EToken.IDENT ||
      token2 === EToken.HASH ||
      token2 === EToken.FUNCTION ||
      token2 === EToken.URL ||
      token2 === EToken.BAD_URL ||
      (token2 === EToken.DELIM &&
        (t2.src === '-' || t2.src === '*' || t2.src === '.')) ||
      token2 === EToken.LBRACKET ||
      token2 === EToken.NUMBER ||
      token2 === EToken.PERCENTAGE ||
      token2 === EToken.DIMENSION ||
      token2 === EToken.UNICODE_RANGE ||
      token2 === EToken.CDC ||
      token2 === EToken.LPAREN
    );
  }
  ...
  if (token1 === EToken.DELIM) {
    ...
    if (src1 === '/') return token2 === EToken.DELIM && t2.src === '*';
...
export class Token implements T.INode {
  ...
  constructor(...args: any[]) {
    if (typeof args[0] === 'number') {
      ...
      } else throw new Error('No constructor with provided argument types');
    } else throw new Error('No constructor with provided argument types');
  }
  ...
  hasLeadingWhitespace(): boolean {
    ...
    if (!this.leadingTrivia) return false;

    len = this.leadingTrivia.length;
    for (i = 0; i < len; i++)
      if (this.leadingTrivia[i].token === EToken.WHITESPACE) return true;
  ...
  hasTrailingWhitespace(): boolean {
    ...
    if (!this.trailingTrivia) return false;

    len = this.trailingTrivia.length;
    for (i = 0; i < len; i++)
      if (this.trailingTrivia[i].token === EToken.WHITESPACE) return true;
  ...
  private constructFromStringAndTrivia(
    token: string,
    leadingTrivia: string[],
    trailingTrivia: string[],
  ) {
    ...
    if (isWhiteSpace(cp)) tokenType = EToken.WHITESPACE;
    else if (cp === EChar.QUOT || cp === EChar.APOS) tokenType = EToken.STRING;
    ...
    else if (cp === EChar.LPAREN) tokenType = EToken.LPAREN;
    else if (cp === EChar.RPAREN) tokenType = EToken.RPAREN;
      ...
      if (wouldStartNumber(token, 0)) {
        ...
        if (rest === '') tokenType = EToken.NUMBER;
        else if (rest === '%') tokenType = EToken.PERCENTAGE;
        else tokenType = EToken.DIMENSION;
      } else if (cp === EChar.HYPHEN && wouldStartIdentifier(token, 0))
        tokenType = EToken.IDENT;
      else tokenType = EToken.DELIM;
    } else if (cp === EChar.COMMA) tokenType = EToken.COMMA;
    else if (cp === EChar.SOLIDUS)
      tokenType = dp === EChar.ASTERISK ? EToken.COMMENT : EToken.DELIM;
    else if (cp === EChar.COLON) tokenType = EToken.COLON;
    else if (cp === EChar.SEMICOLON) tokenType = EToken.SEMICOLON;
    else if (cp === EChar.AT)
      tokenType = wouldStartIdentifier(token, 1)
        ? EToken.AT_KEYWORD
        : EToken.DELIM;
    else if (cp === EChar.LBRACKET) tokenType = EToken.LBRACKET;
    else if (cp === EChar.REVERSE_SOLIDUS)
      tokenType = isEscape(token) ? EToken.IDENT : EToken.DELIM;
    else if (cp === EChar.RBRACKET) tokenType = EToken.RBRACKET;
    else if (cp === EChar.CIRCUMFLEX)
      tokenType = dp === EChar.EQUALS ? EToken.PREFIX_MATCH : EToken.DELIM;
    else if (cp === EChar.LBRACE) tokenType = EToken.LBRACE;
    else if (cp === EChar.RBRACE) tokenType = EToken.RBRACE;
    else if (cp === EChar.UCASE_U || cp === EChar.LCASE_U)
      tokenType =
        dp === EChar.PLUS && (ep === EChar.QUESTION || isHexDigit(ep))
          ? EToken.UNICODE_RANGE
          : EToken.IDENT;
    else if (isNameStart(cp)) tokenType = EToken.IDENT;
    else if (cp === EChar.PIPE) {
      if (dp === EChar.EQUALS) tokenType = EToken.DASH_MATCH;
      else if (dp === EChar.PIPE) tokenType = EToken.COLUMN;
      else tokenType = EToken.DELIM;
  ...
  private constructFromStringAndTokenType(
    token: string,
    tokenType: number,
    leadingTrivia: string[],
    trailingTrivia: string[],
  ...
  private triviaToString(triviaToken: Token[]): string {
    ...
    if (!triviaToken) return '';

    len = triviaToken.length;
    for (i = 0; i < len; i++) s += triviaToken[i].src;
...
export class Tokenizer {
  ...
  constructor(src: string, options?: ITokenizerOptions) {
    ...
    if (this._options.lineBase === undefined) this._options.lineBase = 0;
      ...
      if (t.token === EToken.WHITESPACE || t.token === EToken.COMMENT)
        ...
        if (whitespaceIdx >= 0 && isSignificantWhitespace(currentToken, t)) {
          ...
          if (whitespaceIdx + 1 < len) {
            this._currentToken.trailingTrivia = trailingTrivia.slice(
              whitespaceIdx + 1,
            );
  ...
  private token(
    ...
    return new Token(
      ...
      new AST.SourceRange(
        this._startLine,
        Math.max(0, this._startColumn),
        this._line,
        Math.max(0, this._column),
      ),
  ...
  private getNextToken(): Token {
    ...
    if (cp === EChar.QUOT) return this.consumeString(c, cp);
    ...
    if (cp === EChar.APOS) return this.consumeString(c, cp);
    ...
    if (cp === EChar.PLUS) {
      if (this.wouldStartNumber()) return this.consumeNumeric();
    ...
    if (cp === EChar.HYPHEN) {
      ...
      if (this.wouldStartIdentifier()) return this.consumeIdentLike();
    ...
    if (cp === EChar.LESS_THAN) {
      ...
      if (
        c === '!' &&
        this._src[this._pos + 1] === '-' &&
        this._src[this._pos + 2] === '-'
      ) {
        this.nextChar(); // consume '!'
    ...
    if (isDigit(cp)) return this.consumeNumeric();

    if (cp === EChar.UCASE_U || cp === EChar.LCASE_U) {
      d = this._src.charCodeAt(this._pos + 2);
      if (
        this._src.charCodeAt(this._pos + 1) === EChar.PLUS &&
        (isHexDigit(d) || d === EChar.QUESTION)
      ) {
        // consume 'u'/'U' and '+'
    ...
    if (isNameStart(cp)) return this.consumeIdentLike();
    ...
    // EOF
    if (isNaN(cp)) return this.token(EToken.EOF);
  ...
  private consumeName(): string {
    ...
    for (;;) {
      c = this.nextChar();
      if (isName(this._src.charCodeAt(this._pos))) s += c;
      else if (this.validEscape(c)) s += c + this.consumeEscape();
      else break;
    }
  ...
  private consumeNumber(): string {
    ...
    if (
      (cp === EChar.UCASE_E || cp === EChar.LCASE_E) &&
      (isDigit(d) ||
        ((d === EChar.PLUS || d === EChar.HYPHEN) &&
          isDigit(this._src.charCodeAt(this._pos + 2))))
    ) {
      s += c;
      if (d === EChar.PLUS || d === EChar.HYPHEN) s += this.nextChar();
  ...
  private consumeURL(fnxName: string): Token {
    ...
    if (isNaN(cp)) return this.token(EToken.URL, fnxName + ')', '');
    ...
    if (cp === EChar.QUOT || cp === EChar.APOS) {
      ...
      if (str.token === EToken.BAD_STRING) return this.consumeBadURLRemnants();
    ...
    for (s = ''; ; ) {
      ...
      // create a <bad-url-token>, and return it.
      if (cp === EChar.REVERSE_SOLIDUS) {
        if (this.validEscape(c)) s += c + this.consumeEscape();
        else return this.consumeBadURLRemnants();
      } else {
        // anything else:
  ...
  private consumeBadURLRemnants(): Token {
    ...
    for (;;) {
      ...
      if (this.validEscape(c)) this.consumeEscape();
    }
  }
  ...
  private consumeUnicodeRange(start: string): Token {
    ...
    if (
      this._src.charCodeAt(this._pos) === EChar.HYPHEN &&
      isHexDigit(this._src.charCodeAt(this._pos + 1))
    ) {
      // step 2.1:
