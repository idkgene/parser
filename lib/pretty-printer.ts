...
function beautifyTokenComments(
  triviaToken: Tokenizer.Token[] /*, addSpaces: boolean*/,
) {
  var s = '',
    len: number,
    i: number,
  ...
  if (!triviaToken || triviaToken.length === 0) return '';

  // skip trailing whitespaces
  for (len = triviaToken.length; len > 0; len--)
    if (triviaToken[len - 1].token !== Tokenizer.EToken.WHITESPACE) break;

  // find the first non-whitespace
  for (i = 0; i < len; i++)
    if (triviaToken[i].token !== Tokenizer.EToken.WHITESPACE) break;
  ...
  switch (token.token) {
    case Tokenizer.EToken.WHITESPACE:
      if (start[start.length - 1] === ' ') {
        if (end[0] === ' ') return start + end.substr(1);
        return start + end;
      }
    ...
    case Tokenizer.EToken.COLON:
      ...
      return (
        ...
        (AST.hasParent(token, AST.Selector) ? '' : ' ')
      );
