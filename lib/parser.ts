...
export function parseRule(
  ...
  ret =
    t.token === Tokenizer.EToken.AT_KEYWORD
      ? parser.parseAtRule()
      : parser.parseQualifiedRule();
...
export function parseDeclaration(
  ...
  if (t.token === Tokenizer.EToken.EOF) {
    if (!t.leadingTrivia) return null;
  ...
  if (!ret) ret = parser.parseDeclaration(false);
...
function getLastToken(n: T.INode): Tokenizer.Token {
  var children: T.INode[], len: number;

  if (n instanceof Tokenizer.Token) return <Tokenizer.Token>n;
...
export class Parser {
  ...
  constructor(src: string, options?: Tokenizer.ITokenizerOptions) {
    if (atRules.length === 0) initAtRules();
  ...
  parseStyleSheet(): AST.StyleSheet {
    ...
    return <AST.StyleSheet>(
      new AST.StyleSheet(ruleList || new AST.RuleList([], null, null), cdo, cdc)
    );
  }
  ...
  parseRuleList(isBlock?: boolean): AST.RuleList {
    ...
    for (;;) {
      ...
      if (token === Tokenizer.EToken.EOF) break;
    ...
    return rules.length > 0 || lbrace || rbrace
      ? new AST.RuleList(rules.length > 0 ? rules : null, lbrace, rbrace)
      : null;
  }
  ...
  parseQualifiedRule(): AST.Rule {
    ...
    return selectorList || declarationList
      ? new AST.Rule(selectorList, declarationList)
      : null;
  }
  ...
  parseAtRule(): AST.AtRule {
    ...
    preludeValues = this.parseComponentValueList(
      Tokenizer.EToken.SEMICOLON,
      Tokenizer.EToken.LBRACE,
    );
    ...
    if (spec) {
      if (spec.type === EAtRule.DECLARATION_LIST) {
        try {
          ...
          blockOrSemicolon = AST.DeclarationList.fromErrorTokens(
        ...
        try {
          ...
          blockOrSemicolon = AST.RuleList.fromErrorTokens(
            this.cleanup(e, [Tokenizer.EToken.RBRACE], []),
          );
        }
        ...
        if (this._currentToken.token === Tokenizer.EToken.SEMICOLON) {
          ...
          this.nextToken();
        } else blockOrSemicolon = undefined;
      }

      return atKeyword || prelude || blockOrSemicolon
        ? new spec.astClass(atKeyword, prelude, blockOrSemicolon)
        : null;
    }
    ...
    return atKeyword || prelude || blockOrSemicolon
      ? new AST.AtRule(atKeyword, prelude, blockOrSemicolon)
      : null;
  }
  ...
  parseDeclarationList(): AST.DeclarationList {
    ...
    disabledDeclarations =
      this.parseTrailingTokensForDisabledDeclarations(lbrace);
    ...
    for (;;) {
      ...
      if (token === Tokenizer.EToken.EOF) break;
        ...
        disabledDeclarations = this.parseTrailingTokensForDisabledDeclarations(
          this._currentToken,
        );
        if (disabledDeclarations && disabledDeclarations.length > 0)
          declarations = declarations.concat(disabledDeclarations);
        ...
        try {
          ...
          declarations.push(
            AST.Declaration.fromErrorTokens(
              this.cleanup(e, [Tokenizer.EToken.SEMICOLON], []),
            ),
          );
          prevIsDeclaration = false;
        }
    ...
    return new AST.DeclarationList(
      declarations.length > 0 ? declarations : null,
      lbrace,
      rbrace,
    );
  }
  ...
  parseDeclaration(
    ...
    nameValues = this.parseComponentValueList(
      Tokenizer.EToken.COLON,
      Tokenizer.EToken.SEMICOLON,
      Tokenizer.EToken.RCOMMENT,
    );
    name =
      nameValues && nameValues.length > 0
        ? new AST.ComponentValueList(nameValues)
        : null;

    // If the current input token is anything other than a <colon-token>,
    // this is a parse error. Return nothing.
    // Otherwise, consume the next input token.
    if (throwErrors) this.expect(Tokenizer.EToken.COLON, name);
    ...
    if (this._currentToken.token === Tokenizer.EToken.SEMICOLON) {
      ...
      if (!omitSemicolon) this.nextToken();
    }
    ...
    return name || colon || value || semicolon
      ? new AST.Declaration(name, colon, value, semicolon, lcomment, rcomment)
      : null;
  }
  ...
  parseTrailingTokensForDisabledDeclarations(
    ...
    if (!token.trailingTrivia) return null;
    ...
    for (i = 0; i < len; i++) {
      ...
      if (t.token === Tokenizer.EToken.COMMENT) {
        ...
        if (declaration) {
          ...
          if (!lastToken.trailingTrivia) lastToken.trailingTrivia = [];
    ...
    if (lastToken !== token) updateRange(lastToken);
   ...
   * @returns {*}
   */
  parseDisabledDeclaration(
    token: Tokenizer.Token,
    throwErrors: boolean = true,
  ): AST.Declaration {
    var declaration: AST.Declaration;

    if (token.token !== Tokenizer.EToken.COMMENT) return null;

    try {
      declaration = new Parser(token.src, {
        tokenizeComments: true,
      }).parseDeclaration(throwErrors);

      // ignore if no closing comment was found
      if (!declaration.getRComment()) return null;

      if (declaration)
        Utilities.offsetRange(
          declaration,
  ...
  parseDeclarationValue(): AST.DeclarationValue {
    var values = this.parseComponentValueList(
      Tokenizer.EToken.SEMICOLON,
      Tokenizer.EToken.RBRACE,
      Tokenizer.EToken.RCOMMENT,
    );
    return values && values.length > 0
      ? new AST.DeclarationValue(values)
      : null;
  }
  ...
  parseSelector(): AST.Selector {
    ...
    try {
      for (t = this._currentToken; ; ) {
        ...
        if (token === Tokenizer.EToken.COMMA) {
          ...
          token === Tokenizer.EToken.LPAREN ||
          token === Tokenizer.EToken.LBRACKET
        ) {
          value = this.parseBlock();
          if (value) {
            if (token === Tokenizer.EToken.LBRACKET) {
              value = new AST.AttributeSelector(
                <AST.BlockComponentValue>value,
                namespace,
                pipe,
              );
              namespace = null;
          ...
          token === Tokenizer.EToken.WHITESPACE ||
          (token === Tokenizer.EToken.DELIM &&
            (t.src === '+' || t.src === '>' || t.src === '~'))
        ) {
          ...
          for (i = 0; i < len; i++)
            errorTokens = errorTokens
              ? errorTokens.concat(values[i].getTokens())
              : values[i].getTokens();
    ...
    return (values && values.length > 0) || separator
      ? <AST.Selector>new AST.Selector(values, separator)
      : null;
  }
    ...
    try {
      for (t = this._currentToken; ; ) {
        ...
        if (token === Tokenizer.EToken.EOF || endTokens.indexOf(token) >= 0)
          ...
          if (
            t.token === Tokenizer.EToken.IDENT &&
            t.src.toLowerCase() === 'important'
          ) {
  ...
  parseBlock(): AST.BlockComponentValue {
    ...
    this.expect(
      Tokenizer.EToken.LBRACE,
      Tokenizer.EToken.LBRACKET,
      Tokenizer.EToken.LPAREN,
    );
    this.nextToken();
  ...
  parseFunction(): AST.FunctionComponentValue {
    ...
    return name || t || args.length > 0
      ? new AST.FunctionComponentValue(name, t, args)
      : null;
  }
  ...
  private expect(...args: any[]): void {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (arg instanceof AST.ASTNode || arg instanceof Tokenizer.Token)
        ...
        for (j = 0; j < l; j++) {
          a = arg[j];
          if (a instanceof AST.ASTNode) parsedNodes.push(a);
          else tokens.push(a);
        }
  ...
  private rethrow(e: IParseError, nodes: T.INode[]): void {
    ...
    if (!e.parsedNodes) e.parsedNodes = nodes;
    else e.parsedNodes = nodes.concat(e.parsedNodes);
  ...
  private cleanup(
    ...
    if (e.parsedNodes) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (node instanceof Tokenizer.Token) errorTokens.push(node);
        else if (node instanceof AST.ASTNode) {
          tokens = node.getTokens();
          lenTokens = tokens.length;
          for (j = 0; j < lenTokens; j++) errorTokens.push(tokens[j]);
        }
      }
    }
    ...
    for (t = this._currentToken; ; ) {
      ...
      if (endTokens.indexOf(token) >= 0) break;
    }
  ...
  private getAtRuleSpec(atKeyword: Tokenizer.Token): IAtRuleSpec {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (spec.keyword === value) return spec;
