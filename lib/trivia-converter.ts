  ...
  for (i = 0; i < len; i++) {
    ...
    if (prevToken) {
      ...
      if (!token.leadingTrivia || token.leadingTrivia.length === 0)
        ...
        token.leadingTrivia = prevToken.trailingTrivia.concat(
          token.leadingTrivia,
        );
      ...
      if (token.leadingTrivia && token.leadingTrivia.length > 0) {
        ...
        if (prevParent !== parent) {
          for (; parent; parent = parent.getParent()) {
            if (
              parent.range.startLine === oldStartLine &&
              parent.range.startColumn === oldStartColumn
            ) {
              parent.range.startLine = startLine;
              parent.range.startColumn = startColumn;

              // force a recompute of the tokens and children arrays
              if ((<any>parent)._tokens) (<any>parent)._tokens = null;
              if ((<any>parent)._children) (<any>parent)._children = null;
            } else break;
          }

          for (; prevParent; prevParent = prevParent.getParent()) {
            if (
              prevParent.range.endLine === oldStartLine &&
              prevParent.range.endColumn === oldStartColumn
            ) {
